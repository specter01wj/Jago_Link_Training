...
/*app.post('/formValid', function(req, res) {
    console.log("Req: "+req.body.name);
    res.json(customers);
    // res.json(500, { error: 'An error has occurred!' });
});*/

app.get('/europe/:area', function(req, res, next) {
    var countryId = parseInt(req.params.area);
    var data = {};
    for (var i = 0,len = europe.length;i<len;i++) {
        if (europe[i].area === countryId) {
           data = europe[i];
           break;
        }
    }  
...
app.get('/europe', function(req, res, next) {
    res.json(europe);
    // res.json(500, { error: 'An error has occurred!' });
});

app.get('/asia', function(req, res, next) {
    res.json(asia);
    // res.json(500, { error: 'An error has occurred!' });
});

app.get('/asia/:area', function(req, res, next) {
    var countryId = parseInt(req.params.area);
    var data = {};
    for (var i = 0,len = asia.length;i<len;i++) {
        if (asia[i].area === countryId) {
           data = asia[i];
           break;
        }
    }  
